...
function[outputImage]=ImageSimControllerFuncEvL(proteinPositions,protoLengths)
%% Initializations
% clear all;
% close all;
...
lambdaB = 475;%nm (to show where lat binding is...)
...
if(nBackFluors >0)
    ...
    pointsB=[rand(nBackFluors,1)*(imSize(1)-1)+1,rand(nBackFluors,1)*(imSize(2)-1)+1];%added by SJG 2-2-23
else
    pointsR = [];
    pointsG = [];
    pointsB = [];%added by SJG 2-2-23
end
    ...
    otherwise 
        ...
        for ii=1:numI
            for jj=1:numJ
                ...
                fluorPoints = tubPoints(fluorIdxs,:);
                fluorPoints = nm2pixels(fluorPoints,pixSize);
                greenPointsDimers=proteinPositions(proteinPositions(:,3)==1,1:2);%plot just edge as green
                ...
                bluePointsDimers=proteinPositions(proteinPositions(:,3)==2,1:2);%plot just lat as green %added by SJG 2-2-23
                blueNmPoints=getEB1Coords(bluePointsDimers,rotationRads,'rotationOption','center');%added by SJG 2-2-23
                blueFluorPoints=nm2pixels(blueNmPoints,pixSize);%added by SJG 2-2-23
                %shift MT to desired location
                numMTPoints = size(fluorPoints,1);
                greenNumMtPoints=size(greenFluorPoints,1);
                blueNumMtPoints=size(blueFluorPoints,1);%added by SJG 2-2-23
                ...
                blueFluorPoints=blueFluorPoints+[zeros(blueNumMtPoints,1)+xShift,zeros(blueNumMtPoints,1)+yShift];%added by SJG 2-2-23
                ...
                pointsB=vertcat(pointsB,blueFluorPoints);
            end
        end
