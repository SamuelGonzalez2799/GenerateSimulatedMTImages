function tubCoords=getTubulinCoords(pfLengths,angleRads,varargin)
...
%tic;
nTubTot=sum(pfLengths);
tubCoords=zeros(nTubTot,2);
startIdx=1;
for iPF = 1:PF_NUM
    nTub = pfLengths(iPF);%sum(pfLengths);
    idxArray = 1:nTub;
    tubCoords(startIdx:nTub+startIdx-1,1)=idxArray*TUB_LENGTH;
    tubCoords(startIdx:nTub+startIdx-1,2)=FLUOR_DIST_FROM_MT_CENTER*sin(2*pi*(iPF-1)/13);
    startIdx = nTub+startIdx;
end
tubCoords(startIdx:end,:)=[];
...
if(angleRads~=0)
    switch rotationOption
        case 'minusend'
            ...
            tubCoords = tubCoords*rotMatrix;
        case 'center'
            maxVal = max(tubCoords(:,1));
            tubCoords(:,1) = tubCoords(:,1) - maxVal/2;
            rotMatrix = [cos(angleRads),sin(angleRads); -sin(angleRads),cos(angleRads)];
            tubCoords = tubCoords*rotMatrix;
            tubCoords(:,1) = tubCoords(:,1) + maxVal/2;
        case 'plusend'
            maxVal = max(tubCoords(:,1));
            tubCoords(:,1) = tubCoords(:,1) - maxVal;
            rotMatrix = [cos(angleRads),sin(angleRads); -sin(angleRads),cos(angleRads)];
            tubCoords = tubCoords*rotMatrix;
            tubCoords(:,1) = tubCoords(:,1) + maxVal;
    end
